...
interface RescheduleModalProps {
    isOpen: boolean;
    onClose: () => void;
    onReschedule?: (data: { selectedDate: Date | null; selectedTime: string; selectedTimezone: string }) => void;
...
export default function RescheduleModal({
    ...
    const [currentDate, setCurrentDate] = useState(() => event ? event.date : new Date());
    const [selectedDate, setSelectedDate] = useState<Date | null>(() => event ? event.date : new Date());
    const [selectedTime, setSelectedTime] = useState<string>(() => event ? event.time : '');
    ...
    // Profile images - set to null if not available, will fallback to initials
    const abdullahImage: string | null = null; // Add image URL here when available
    const elenaImage: string | null = null; // Add image URL here when available
    ...
    const handleReschedule = () => {
        ...
        if (onReschedule) {
            onReschedule({ selectedDate, selectedTime, selectedTimezone });
        }
        onClose();
    };
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center px-4 py-4">
            ...
            <div className="relative w-full max-w-4xl max-h-[90vh] flex flex-col" onClick={(e) => e.stopPropagation()}>
                <div className="relative bg-white rounded-lg shadow-xl flex flex-col max-h-full overflow-hidden">
                    ...
                    <div className="overflow-y-auto flex-1 scroll-hidden">
                        <div className="px-6 pb-6">
                            <div className="flex gap-6">
                                {/* Group Availability Panel - Left (Separate) */}
                                <div className="w-[25%] border border-[#E4E7EC] rounded-lg px-2 py-6">
                                    <h3 className="text-md font-bold text-[#2E2C34] mb-4">Group Availability</h3>
                                    ...
                                    <div className="space-y-2">
                                        {/* Abdullah Mehdi */}
                                        <button 
                                            onClick={() => setSelectedMember('abdullah')}
                                            className={`w-full flex items-center gap-3 px-3 py-2 rounded-sm transition-colors border ${
                                                selectedMember === 'abdullah'
                                                    ? 'bg-[#E9E8FB] border-l-4 border-l-[#5542F6] border-r border-t border-b border-[#E4E7EC]'
                                                    : 'hover:bg-[#F7F8FA] border border-[#E4E7EC]'
                                            }`}
                                        >
                                            <div className="w-10 h-10 rounded-full bg-[#E9E8FB] flex items-center justify-center shrink-0 border border-[#E4E7EC] overflow-hidden">
                                                {abdullahImage ? (
                                                    <img src={abdullahImage} alt="Abdullah Mehdi" className="w-full h-full object-cover" />
                                                ) : (
                                                    <span className="text-sm font-semibold text-[#5542F6]">AM</span>
                                                )}
                                            </div>
                                            <span className={`text-sm ${selectedMember === 'abdullah' ? 'font-medium text-[#5542F6]' : 'text-[#24282E]'}`}>
                                                Abdullah Mehdi
                                            </span>
                                        </button>
                                        ...
                                        {/* Elena Perry */}
                                        <button 
                                            onClick={() => setSelectedMember('elena')}
                                            className={`w-full flex items-center gap-3 px-3 py-2 rounded-sm transition-colors border ${
                                                selectedMember === 'elena'
                                                    ? 'bg-[#E9E8FB] border-l-4 border-l-[#5542F6] border-r border-t border-b border-[#E4E7EC]'
                                                    : 'hover:bg-[#F7F8FA] border border-[#E4E7EC]'
                                            }`}
                                        >
                                            <div className="w-10 h-10 rounded-full bg-[#FFE5E5] flex items-center justify-center shrink-0 border border-[#E4E7EC] overflow-hidden">
                                                {elenaImage ? (
                                                    <img src={elenaImage} alt="Elena Perry" className="w-full h-full object-cover" />
                                                ) : (
                                                    <span className="text-sm font-semibold text-[#FF6B6B]">EP</span>
                                                )}
                                            </div>
                                            <span className={`text-sm ${selectedMember === 'elena' ? 'font-medium text-[#5542F6]' : 'text-[#24282E]'}`}>
                                                Elena Perry
                                            </span>
                                        </button>
                                        ...
                                        >
                                            <div className="relative flex shrink-0">
                                                <div className="w-10 h-10 rounded-full bg-[#E9E8FB] border-2 border-white flex items-center justify-center">
                                                    <span className="text-xs font-semibold text-[#5542F6]">AM</span>
                                                </div>
                                                <div className="w-10 h-10 rounded-full bg-[#FFE5E5] border-2 border-white flex items-center justify-center -ml-2">
                                                    <span className="text-xs font-semibold text-[#FF6B6B]">EP</span>
                                                </div>
                                            </div>
                                            <span className={`text-sm ${selectedMember === 'all' ? 'font-medium text-[#24282E]' : 'text-[#24282E]'}`}>
                                                All Members
                                            </span>
                                        </button>
                                    </div>
                                ...
                                {/* Calendar and Time Slot Section - Right (Together) */}
                                <div className="flex-1 border border-[#E4E7EC] rounded-lg p-6 flex relative">
                                    ...
                                    <div>
                                        ...
                                        <div className="flex items-center gap-2 py-2.5">
                                            ...
                                            <span className="text-[0.873rem] text-[#84818A]">{event.client}</span>
                                        </div>
                                    </div>
                                </div>
                    ...
                    {/* Footer */}
                    <div className="flex justify-end px-6 py-6 shrink-0">
                        <button
