...
export default function AIAssistancePage() {
    ...
    return (
        <AppLayout activeKey="ai-assistant" contentClassName="h-[calc(100vh-64px)] overflow-hidden">
            ...
            <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-3 sm:gap-0 mt-2 sm:mt-4">
                <div>
                    <div className="text-xs sm:text-sm text-[#727A90]">Home / <span className="text-[#24282E]">AI Assistant</span></div>
                    <h1 className="text-xl sm:text-2xl lg:text-[28px] font-semibold text-[#24282E] mt-1 sm:mt-2">AI Assistant</h1>
                </div>
                <button onClick={() => setShowFeatures(true)} className="inline-flex items-center gap-2 bg-white border border-[#E4E7EC] rounded-md px-2 sm:px-3 py-1.5 sm:py-2 text-xs sm:text-sm font-semibold text-[#24282E] hover:bg-[#F7F8FA] self-start sm:self-auto">
            ...
            <div className="mt-3 sm:mt-4 flex flex-col flex-1 min-h-0">
                <div className="flex-1 overflow-y-auto space-y-3 sm:space-y-4 pr-2 sm:pr-6">
                {messages.map((m) => {
                    if (m.role === 'user') {
                        return (
                            <div key={m.id} className="flex justify-end">
                                <div className="bg-primary text-white max-w-[85%] sm:max-w-[70%] rounded-md px-3 py-2 text-xs sm:text-sm break-words">{m.text}</div>
                            </div>
                        );
                    }
                    // assistant
                    return (
                        <div key={m.id} className="flex justify-start">
                            <div className="w-6 h-6 sm:w-7 sm:h-7 rounded-full overflow-hidden mr-2 shrink-0 mt-0.5 bg-linear-to-br from-[#7B61FF] to-[#C33DF0] grid place-items-center">
                                <AiSvg />
                            </div>
                            {m.kind === 'buttons' ? (
                                <div className="bg-white text-[#24282E] border border-[#E4E7EC] max-w-[85%] sm:max-w-[80%] rounded-md px-3 sm:px-4 py-2 sm:py-3 text-xs sm:text-sm">
                                    <div className="mb-2 sm:mb-3 leading-relaxed">{m.text}</div>
                                    <div className="flex flex-wrap gap-2 sm:gap-3">
                                        {m.buttons.map((b, i) => (
                                            <span key={i} className="px-3 sm:px-4 py-1.5 sm:py-2 bg-white border border-[#E4E7EC] rounded-md text-xs sm:text-sm text-[#24282E]">
                                                {b}
                                            </span>
                                        ))}
                                    </div>
                                ...
                                <div className="bg-white text-[#24282E] border border-[#E4E7EC] rounded-md p-3 sm:p-4 max-w-[85%] sm:max-w-[80%]">
                                    <div className="text-base sm:text-xl font-semibold mb-3 sm:mb-4">{m.title}</div>
                                    <Image src={m.imgSrc} alt="Form preview" width={240} height={240} className="rounded-md border border-[#E4E7EC] w-full sm:w-auto h-auto max-w-full" />
                                </div>
                            )}
                        </div>
                    );
            ...
            <div className="mt-2 sm:mt-3">
                <div className="bg-white border border-[#E4E7EC] rounded-lg h-32 sm:h-40 p-3 sm:p-4 flex flex-col">
                    ...
                    <textarea
                        ...
                        className={`w-full resize-none outline-none text-sm sm:text-md placeholder-[#9AA0A6] flex-1 ${
                            inputValue.trim().startsWith('/') ? 'text-transparent caret-[#24282E]' : 'text-[#24282E]'
                        }`}
                    />
                    <div className="mt-2 sm:mt-3 flex justify-end">
                        <button onClick={handleSend} className="h-9 sm:h-10 px-2 sm:px-3 bg-primary text-white rounded-md inline-flex items-center gap-1 sm:gap-2">
            ...
            <div className={`fixed inset-0 z-70 pointer-events-none`}>
                <div className={`absolute top-0 right-0 h-full transition-transform duration-200 ${showFeatures ? 'translate-x-0' : 'translate-x-full'} pointer-events-auto`}>
                    <div className="h-full w-full sm:w-[380px] p-3 sm:p-4 flex">
